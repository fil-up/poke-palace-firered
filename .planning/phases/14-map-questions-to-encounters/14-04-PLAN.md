...
// Note: Quiz_CountMasteredQuestionsForSpecies uses current section internally
u8 mastered = Quiz_CountMasteredQuestionsForSpecies(species);
if (mastered >= 5)
{
...
Note: Quiz_CountMasteredQuestionsForSpecies() is implemented in Task 2 below.
  </action>
...
2. Add declaration to quiz.h:
```c
// In pokefirered/include/quiz/quiz.h, add:
u8 Quiz_CountMasteredQuestionsForSpecies(u16 species);
```

3. Update or add Quiz_CountMasteredQuestions to be section-aware:
```c
u8 Quiz_CountMasteredQuestionsForSpecies(u16 species)
{
...
4. Update the "all questions mastered" check in Quiz_OnMoveConfirmed:
...
5. Update Quiz_GetProgress() and Quiz_GetMasteryProgress() for UI display:
```c
...
6. Update capture success to mark cleared in section:
```c
...
3. For gym leaders, use Quiz_GetGymConfig to get the gym's mastery pool:
```c
static u8 Quiz_BuildGymMasteryPool(u8 gymId)
{
    const struct QuizGymConfig *gymConfig;
    const struct QuizTopicPool *pool;
    u8 count = 0;
    u8 i, j;
    
    gymConfig = Quiz_GetGymConfig(gymId);
    if (gymConfig == NULL)
        return Quiz_BuildAreaPool();  // Fallback to map topics
    
    // Add questions from all mastery pools
    for (i = 0; i < gymConfig->poolCount && count < MAX_POOL_SIZE; i++)
    {
        pool = gymConfig->masteryPools[i];
        if (pool == NULL)
            continue;
